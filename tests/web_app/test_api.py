...
def create_connect4_game():
    response = client.post(
        "/games/new", json={"game_type": "connect4", "player_options": {"1": {"player_type": "human"}}}
    )
    assert response.status_code == 200
    data = response.json()
...
def create_othello_game():
    response = client.post(
        "/games/new", json={"game_type": "othello", "player_options": {"1": {"player_type": "human"}}}
    )
    assert response.status_code == 200
    data = response.json()
    assert "game_id" in data
...
def test_make_move_othello():
    ...
    response = client.post(f"/games/{game_id}/move", json={"row": move_row, "col": move_col})
    assert response.status_code == 200
    data = response.json()
