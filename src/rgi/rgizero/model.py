...
class Block(nn.Module):
    """A single transformer layer with residual connections."""
...
class TransformerConfig:
    n_max_context: int = 1024  # max context length
...
class Transformer(nn.Module):
    """Internal layers of a transformer with embedding tensors as inputs and outputs."""

    def __init__(self, config: TransformerConfig):
        super().__init__()
        self.config = config

        self.wpe = nn.Embedding(config.n_max_context, config.n_embd)
        self.dropout = nn.Dropout(config.dropout)
        self.blocks = nn.ModuleList([Block(config) for _ in range(config.n_layer)])

    def forward(self, x: torch.Tensor):  # x: (b, t, n_embd)
        device = x.device
        b, t, n_embd = x.size()
        assert t <= self.config.n_max_context, (
            f"Cannot forward sequence of length {t}, block size is only {self.config.n_max_context}"
        )
        assert n_embd == self.config.n_embd, (
            f"Input embedding dimension {n_embd} does not match model embedding dimension {self.config.n_embd}"
        )
