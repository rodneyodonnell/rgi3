...
from rgi.rgizero.players.alphazero import (
    ...
    MCTSStats,
    SearchResult,
    validate_array_probabilities_or_die,
)
...
class InferenceServer:
    ...
    def _process_batch(self, batch: list[EvalRequest]):
        ...
        if self.verbose and self.total_batches % 100 == 0:
            ...
            print(f"InferenceServer: {self.total_batches} batches, mean_size={mean_batch:.1f}, evals/sec={mean_evals_per_sec:.1f}")
...
class BatchInferenceServer:
    ...
    def _process_batch_requests(self, batch_requests: list[BatchEvalRequest], response_queues: dict[int, mp.Queue]):
        ...
        if self.verbose and self.total_batches % 50 == 0:
            ...
            print(f"BatchInferenceServer: {self.total_batches} mega-batches, mean_size={mean_batch:.1f}, evals/sec={evals_per_sec:.1f}")
...
def async_worker_process(
    ...
    import asyncio
    asyncio.run(async_worker_process_main(
        worker_id, game_name, num_games, num_simulations, concurrent_games,
        request_queue, response_queue, result_queue, seed
    ))
    ...
    def play_games(self, total_games: int, seed: int = 42) -> list:
        ...
        for i in range(self.num_workers):
            ...
            p = mp.Process(
                ...
                args=(
                    i, self.game_name, num_games, self.num_simulations,
                    self.concurrent_games_per_worker,
                    request_queue, response_queues[i], result_queue, worker_seed,
                ),
            )
            workers.append(p)
            p.start()
